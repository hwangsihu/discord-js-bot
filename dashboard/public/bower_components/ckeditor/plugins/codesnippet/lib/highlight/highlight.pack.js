var hljs = new (function () {
  ...
  function r(w) {
    ...
    v = v.map((x) => x.replace(/^language-/, ""));
    return v.filter((x) => j(x) || x == "no-highlight")[0];
  }
  function o(x, y) {
    var v = {};
    for (var w in x) {
  ...
  function h(v) {
    if (b.tabReplace) {
      v = v.replace(/^((<[^>]+>|\t)+)/gm, (w, z, y, x) => z.replace(/\t/g, b.tabReplace));
    }
    if (b.useBR) {
      v = v.replace(/\n/g, "<br>");
    }
...
hljs.registerLanguage("php", (b) => {
  ...
  return {
    ...
    c: [
      b.CLCM,
      b.HCM,
      { cN: "comment", b: "/\\*", e: "\\*/", c: [{ cN: "phpdoc", b: "\\s@[A-Za-z]+" }, a] },
...
hljs.registerLanguage("objectivec", (a) => {
  ...
  return {
    ...
    c: [
      ...
      { cN: "preprocessor", b: "#", e: "$" },
      { cN: "class", b: "(" + b.split(" ").join("|") + ")\\b", e: "({|$)", k: b, l: c, c: [a.UTM] },
...
hljs.registerLanguage("nginx", (c) => {
  ...
  return {
    c: [
      c.HCM,
      { b: c.UIR + "\\s", e: ";|{", rB: true, c: [c.inherit(c.UTM, { starts: a })], r: 0 },
    ],
    i: "[^\\s\\}]",
  };
});
...
hljs.registerLanguage("apache", (a) => {
  var b = { cN: "number", b: "[\\$%]\\d+" };
  return {
    cI: true,
    c: [
...
hljs.registerLanguage("cpp", (a) => {
  ...
  return {
    ...
    c: [
      ...
      a.CNM,
      { cN: "preprocessor", b: "#", e: "$", c: [{ b: "include\\s*<", e: ">", i: "\\n" }, a.CLCM] },
      {
        cN: "stl_container",
