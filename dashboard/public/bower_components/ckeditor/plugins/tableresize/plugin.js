...
(function () {
  ...
  function E(b) {
    ...
    function a() {
      for (var c = e.rtl, l = c ? u.length : t.length, a = 0, f = 0; f < l; f++) {
        ...
        CKEDITOR.tools.setTimeout(
          ...
          0,
          this,
          [g, g && x(g), d, d && x(d), (!g || !d) && x(m) + r(m, "left") + r(m, "right"), h]
        );
      }
    }
    function l(l) {
    ...
    this.attachTo = function (b) {
      ...
      m ||
        ...
        (d = a.y),
        (a = f.height + f.y - a.y),
        c.setStyles({ width: k(b.width), height: k(a), left: k(b.x), top: k(d) }),
        w && c.setOpacity(0.25),
  ...
  function y(b) {
    ...
    if ("mouseout" == b.name) {
      ...
      for (
        var a = new CKEDITOR.dom.element(b.data.$.relatedTarget || b.data.$.toElement);
