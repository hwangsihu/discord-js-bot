              ...
              (f = c[h]),
                ...
                -1 === l(b.preservedElements, f.element)
                  ? CKEDITOR.env.webkit && !CKEDITOR.env.chrome
                    ? c[h].removeFromRange(d.data.range, d.editor)
                    : c[h].remove(d.editor)
                  : u(f.element, d.data.styles) && b._removeStylesFromElementInRange(e, f.element),
                e.moveToBookmark(g);
        }
      });
      a.copyFormatting.on(
        ...
        (b) => {
          ...
          "list" === c && a.copyFormatting._isContextAllowed("list")
            ? e._applyStylesToListContext(b.editor, b.data.range, b.data.styles)
            : "table" === c && a.copyFormatting._isContextAllowed("table")
              ? e._applyStylesToTableContext(b.editor, b.data.range, b.data.styles)
              : a.copyFormatting._isContextAllowed("text") &&
                e._applyStylesToTextContext(b.editor, b.data.range, b.data.styles);
        },
        null,
        null,
        999
        ...
        exec: function (a, b) {
          ...
          if (this.state === CKEDITOR.TRISTATE_ON)
            return (
              ...
              d._putScreenReaderMessage(a, "canceled"),
              d._detachPasteKeystrokeHandler(a),
          ...
          c ||
            ...
            a.config.copyFormatting_outerCursor && h.addClass("cke_copyformatting_disabled"));
          e.sticky = f;
    ...
    _getSelectedWordOffset: (a) => {
      ...
      function d(a) {
        return a.type == CKEDITOR.NODE_ELEMENT
          ? ((a = a.getHtml().replace(/<span.*?>&nbsp;<\/span>/g, "")), a.replace(/<.*?>/g, ""))
          : a.getText();
      }
      function e(a, c) {
        var f = a,
      ...
      for (f = d(h); null != (g = c.exec(f)); )
        if (g.index + g[0].length >= a.startOffset)
          return (
            ...
            0 === g.index && ((g = e(h, !0)), (m = g.node), (a = g.offset)),
            c >= f.length && ((f = e(h)), (k = f.node), (c = f.offset)),
      ...
      for (f = 0; f < d.length; f++)
        if (((b = d[f]), -1 === l(e.excludedElementsFromInlineTransform, b.element))) {
          if (-1 !== l(e.elementsForInlineTransform, b.element))
            for (b.element = b._.definition.element = "span", g = 0; g < c.length; g++)
              b._.definition.attributes[c[g]] && delete b._.definition.attributes[c[g]];
          b.apply(a);
        }
    },
