...
CKEDITOR.dialog.add("image2", function (f) {
  ...
  function F() {
    ...
    a !== x.data.src
      ? (G(a, function (a, b, c) {
          ...
          u = k = b;
          v = l = c;
          m(H.checkHasNaturalRatio(a));
        }),
        (n = !0))
      : n
        ? (t(!0), g.setValue(k), h.setValue(l), (n = !1))
        : t(!0);
  }
  function I() {
    if (e) {
      var a = this.getValue();
      if (a && (a.match(D) || m(!1), "0" !== a)) {
        var b = "width" == this.id,
          c = k || u,
          d = l || v,
          a = b ? Math.round((a / c) * d) : Math.round((a / d) * c);
  ...
  var D = /(^\s*(\d+)(px)?\s*$)|^$/i,
    ...
    ).output({ lockButtonId: J, resetButtonId: K }),
  ...
  return {
    ...
    contents: [
      ...
      },
      {
        ...
        label: b.uploadTab,
        elements: [
          { type: "file", id: "upload", label: b.btnUpload, style: "height:40px" },
          {
            type: "fileButton",
            id: "uploadButton",
            filebrowser: "info:src",
