if (!document.createElement("canvas").getContext) {
  (() => {
    ...
    function c(i) {
      return Number.parseFloat(i) / 100;
    }
    function r(j, m, i) {
    ...
    function I(ag) {
      var i, ai, aj, ah, ak, Z;
      ah = (Number.parseFloat(ag[0]) / 360) % 360;
      if (ah < 0) {
        ah++;
      }
      ak = r(c(ag[1]), 0, 1);
    ...
    function u(m, j) {
      ...
      var ag = Number.parseFloat(j.currentStyle.fontSize),
        Z = Number.parseFloat(m.size);
      if (typeof m.size == "number") {
        i.size = m.size;
      } else if (m.size.indexOf("px") != -1) {
        i.size = Z;
      } else if (m.size.indexOf("em") != -1) {
        i.size = ag * Z;
      } else if (m.size.indexOf("%") != -1) {
        i.size = (ag / 100) * Z;
      } else if (m.size.indexOf("pt") != -1) {
        i.size = Z / 0.75;
      } else {
        i.size = ag;
      }
    ...
    q.createLinearGradient = (j, p, i, m) => {
    ...
    q.createRadialGradient = (p, ag, m, j, Z, i) => {
    ...
    function G(aq, ai, aK, ar) {
      ...
      if (aj instanceof U) {
        ...
        var av = aj.colors_;
        av.sort((aN, i) => aN.offset - i.offset);
        var ap = av.length;
        var au = av[0].color;
